...
_gulp2['default'].task('views', function () {
  return _gulp2['default'].src('app/templates/pages/*.jade').pipe($.plumber()).pipe($.jade({ pretty: true }).on('error', function (err) {
    console.log(err);
  })).pipe(_gulp2['default'].dest('.tmp')).pipe(reload({ stream: true }));
});
...
_gulp2['default'].task('lint', lint(['app/scripts/**/*.js', '!app/scripts/vendor/**/*.js']));
_gulp2['default'].task('lint:test', lint('test/spec/**/*.js', testLintOptions));

_gulp2['default'].task('html', ['views', 'styles'], function () {
  var assets = $.useref.assets({ searchPath: ['.tmp', 'app', '.'] });

  return _gulp2['default'].src(['app/*.html', '.tmp/*.html']).pipe(assets)
  // .pipe($.if('*.js', $.uglify()))
  // .pipe($.if('*.css', $.minifyCss({compatibility: '*'})))
  .pipe(assets.restore()).pipe($.useref())
...
_gulp2['default'].task('sprite', function () {
  ...
  spriteData.img.pipe(_gulp2['default'].dest('app/images'));

  // Pipe CSS stream through CSS optimizer and onto disk
  spriteData.css.pipe(_gulp2['default'].dest('app/styles/'));
});
...
_gulp2['default'].task('extras', function () {
  return _gulp2['default'].src(['app/*.*', '!app/*.html', '!app/templates/*.jade'], {
    dot: true
  }).pipe(_gulp2['default'].dest('dist'));
});
...
_gulp2['default'].task('watch', ['views', 'styles', 'fonts'], function () {
...
_gulp2['default'].task('watch:dist', function () {
  (0, _browserSync2['default'])({
    notify: false,
    port: 9000,
    server: {
...
_gulp2['default'].task('wiredep', function () {
  ...
  _gulp2['default'].src('app/templates/**/*.jade').pipe((0, _wiredep.stream)({
    exclude: ['bootstrap-sass', 'jquery', 'modernizr'],
    ignorePath: /^(\.\.\/)*\.\./
  })).pipe(_gulp2['default'].dest('app/templates'));
});
